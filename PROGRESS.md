...
- [x] **LLM cost: kill the reservation NL-extraction re-loop + pre-filter mailbox scanning; honest briefing send metric (Ryan cost + metrics flags)**
  - Date: 2026-06-29
  - What changed: Ryan flagged (a) compute cost up "dramatically" and (b) the briefing's send count pulling his "normal emails". Investigated against prod data + code. **Cost root cause:** the reservation ingest re-ran the paid LLM trip extraction (`nl_extraction`) on every email in the GENERAL mailboxes (rlandry@, percy@) every ~10-min cycle, and because a non-booking email is filtered (never persisted), the `graph_message_id` dedup couldn't skip it -- so the SAME normal email was re-extracted every cycle for 72h. Result: **19,809 extraction calls against only 191 reservations = $100 over 8 days (95% of all LLM spend, ~$375/mo run-rate), almost all waste.** Fixes: **(1)** new pure `looksLikeReservationCandidate()` ([reservationClassify.ts](src/services/reservationClassify.ts)) -- a cheap keyword pre-filter; on general mailboxes (`onlyBookings`) an email with no transportation signal never reaches the LLM, so Ryan's normal/relationship mail is never scanned or charged (BookRides + booking-shaped mail still flow; the dedicated ljreservations@ mailbox is never pre-filtered). **(2)** in-process `_seenNonBooking` set ([reservationQuoteService.ts](src/services/reservationQuoteService.ts)) -- a message the LLM already judged non-booking is not re-extracted again this process lifetime (closes the residual loop for booking-shaped-but-rejected mail). **(3) Briefing metric** ([weeklyBriefingService.ts](src/services/weeklyBriefingService.ts) + [weeklyBriefingRenderer.ts](src/services/weeklyBriefingRenderer.ts)): "Logged sends" was an ALL-TIME `COUNT(*) FROM communication_logs WHERE direction='outbound'` shown under a date-range label, so it read as weekly. Added a 7-day window (`sends7d`) and relabeled the subject/text/HTML headline to "campaign sends this week (N to date)" + an explicit note that only outbound CAMPAIGN emails (each tied to a campaign+lead) are counted, never personal mailbox mail.
  - Verification: **Validated Ryan's claim against all 113 rows** -- every one was system-sent (Graph API, delivery_mode=live, status=sent, `graph-` message id), tied to a campaign + lead, templated cold pitch; lead sources apollo(41)/api(38)/investor_database(34), zero referral, zero consumer domains. So NOT his personal Outlook mail -- BUT 52/113 were hand-curated and 34 are his investor network ("Reconnecting…" subjects), so his relationships WERE cold-pitched by the campaign (a targeting issue, not a metric scrape). backend `tsc --noEmit` clean; jest reservationClassify **10/10** (BookRides always passes; booking-shaped passes; normal relationship/newsletter mail skipped; case-insensitive) + reservationQuoteService **35/35**. Deployed; nl_extraction call-rate drop confirmed on prod (see session summary).
  - Notes: the in-process seen-set is cleared on restart (a message is re-examined at most once per process). The remaining product issue -- the campaign emailing Ryan's warm/investor relationships with a generic cold template -- is a lead-list/targeting decision for Ali/Ryan, not a code defect.
