...
- [x] **Outreach area isolation hardening + Grant send-capability + live 25-point release checklist**
  ...
  - Verification: backend `tsc --noEmit` clean; jest [outreachRelease.test.ts](src/tests/unit/outreachRelease.test.ts) **45/45** (adds isStateInScope: code+full-name match, out-of-area blocked, null-state blocked for scoped users). Live 25-point run on prod after deploy: see the session summary / Basecamp comment for the PASS matrix.
