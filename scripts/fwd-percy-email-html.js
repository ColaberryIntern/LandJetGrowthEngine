/**
 * Sends ali@colaberry.com a copy of the email sent to Percy, in proper HTML format
 * (rendered the same way Percy received it).
 */
require('dotenv').config();

const TO = 'ali@colaberry.com';
const SUBJECT = "Fwd: Re: AI In-Bound Email Engine -- Got everything, 3 questions before I lock the pricing engine";
...
const FWD_NOTE = `<div style="font-family:Arial,Helvetica,sans-serif;font-size:13px;color:#666666;background:#f5f7fa;padding:10px 14px;border-left:3px solid #1a8fb5;margin-bottom:16px">
  <strong>Forwarded copy</strong> &middot; Sent to <strong>pkapadia@landjet.com</strong> on 2026-05-06, CC: rlandry@landjet.com
</div>`;

const ORIGINAL_BODY_HTML = `<div style="font-family:Arial,Helvetica,sans-serif;font-size:14px;color:#222222;line-height:1.55;max-width:680px">
  <p>Percy,</p>
...
const FULL_HTML = `<!DOCTYPE html><html><body>${FWD_NOTE}${ORIGINAL_BODY_HTML}${SIGNATURE_HTML}</body></html>`;

const FULL_TEXT = `[Forwarded copy] Sent to pkapadia@landjet.com on 2026-05-06, CC: rlandry@landjet.com
...
async function send() {
  ...
  if (sendResp.ok) {
    console.log('FORWARDED (HTML) to ali@colaberry.com! Message ID:', result.id);
  } else {
