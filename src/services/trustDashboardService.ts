...
function buildRemediation(sig: {
  ...
  const items: RemItem[] = [
    ...
    { id: 'G4', label: 'LLM cost / token observability', status: sig.costInstrumented ? 'partial' : 'open', live: true,
      signal: sig.costInstrumented ? `Instrumented, ${sig.costCalls24h} calls logged (24h); ~6 of ~20 call sites wired` : 'No cost rows', pri: 'P0' },
