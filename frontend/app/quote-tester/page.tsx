...
interface Sample { name: string; email_body: string; }
          ...
          <div className="rounded-lg border border-gray-200 bg-white p-4">
            ...
            {method === 'paste' ? (
              <>
                ...
                {samples.length > 0 && (
                  <div className="mt-3">
                    <div className="text-xs text-gray-500 mb-1.5">Try a sample:</div>
                    <div className="flex flex-wrap gap-1.5">
                      {samples.map(s => (
                        <button key={s.name} onClick={() => setEmailBody(s.email_body)}
                          className="rounded-md border border-gray-200 bg-gray-50 px-2 py-1 text-xs text-gray-700 hover:bg-gray-100">
                          {s.name}
                        </button>
                      ))}
                    </div>
